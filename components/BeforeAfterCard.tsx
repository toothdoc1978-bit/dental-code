...
export function BeforeAfterCard({ item }: { item: GalleryCase }) {
  return (
    <article className="card card-hover flex h-full flex-col gap-4 overflow-hidden p-0">
      <div className="relative aspect-[4/5] w-full bg-brand-50">
        <Media
          ...
          label={item.title}
        />
        <span className="absolute left-3 top-3 z-10 rounded-full bg-white/90 px-3 py-1 text-xs font-semibold text-brand-700 shadow-sm">
          Before / After
        </span>
      </div>
